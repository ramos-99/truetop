...
- Linux 5.15 or newer, the range CI runs the programs on (5.15 through 6.18).
  The pre-5.14 `state`-field path is still in the code, so 5.10 through 5.14 may
  work, but as of today that is neither tested nor guaranteed.
- **x86-64 only.** aarch64 compiles, and CI checks that it still does on every
  push, but the programs have never been loaded on an ARM kernel: no runtime
  test, no entry in the kernel matrix, no released binary. Unsupported until
  that changes - it is a [roadmap](#roadmap) item.
...
`--background <dark|light|auto>` picks the row tints for the terminal's
background. `auto`, the default, asks the terminal over OSC 11 and falls back to
dark where nothing answers, as under tmux.
